VERSION 5.00
Begin VB.Form Form1 
   Caption         =   "Form1"
   ClientHeight    =   3855
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6510
   LinkTopic       =   "Form1"
   ScaleHeight     =   3855
   ScaleWidth      =   6510
   StartUpPosition =   3  'Windows Default
   Begin VB.CommandButton Command1 
      Caption         =   "Enter"
      Height          =   495
      Left            =   3840
      TabIndex        =   0
      Top             =   2880
      Width           =   855
   End
   Begin VB.Line Line61 
      X1              =   2400
      X2              =   2640
      Y1              =   1560
      Y2              =   1320
   End
   Begin VB.Line Line60 
      X1              =   2640
      X2              =   2640
      Y1              =   1320
      Y2              =   1080
   End
   Begin VB.Line Line59 
      X1              =   2880
      X2              =   2640
      Y1              =   960
      Y2              =   1080
   End
   Begin VB.Line Line58 
      X1              =   5640
      X2              =   5400
      Y1              =   1440
      Y2              =   1320
   End
   Begin VB.Line Line57 
      X1              =   5400
      X2              =   5400
      Y1              =   1080
      Y2              =   1320
   End
   Begin VB.Line Line56 
      X1              =   5160
      X2              =   5400
      Y1              =   960
      Y2              =   1080
   End
   Begin VB.Line Line55 
      X1              =   4440
      X2              =   4680
      Y1              =   1200
      Y2              =   1200
   End
   Begin VB.Line Line54 
      X1              =   3960
      X2              =   4200
      Y1              =   1200
      Y2              =   1200
   End
   Begin VB.Line Line53 
      X1              =   4440
      X2              =   4440
      Y1              =   960
      Y2              =   1200
   End
   Begin VB.Line Line52 
      X1              =   4680
      X2              =   4680
      Y1              =   960
      Y2              =   1200
   End
   Begin VB.Line Line51 
      X1              =   4200
      X2              =   4200
      Y1              =   960
      Y2              =   1200
   End
   Begin VB.Line Line50 
      X1              =   3960
      X2              =   3960
      Y1              =   960
      Y2              =   1200
   End
   Begin VB.Line Line49 
      X1              =   3720
      X2              =   3720
      Y1              =   1200
      Y2              =   960
   End
   Begin VB.Line Line48 
      X1              =   3480
      X2              =   3480
      Y1              =   960
      Y2              =   1200
   End
   Begin VB.Line Line47 
      X1              =   4680
      X2              =   4920
      Y1              =   960
      Y2              =   960
   End
   Begin VB.Line Line46 
      X1              =   3480
      X2              =   3720
      Y1              =   1200
      Y2              =   1200
   End
   Begin VB.Line Line45 
      X1              =   4200
      X2              =   4440
      Y1              =   960
      Y2              =   960
   End
   Begin VB.Line Line44 
      X1              =   3720
      X2              =   3960
      Y1              =   960
      Y2              =   960
   End
   Begin VB.Line Line43 
      X1              =   3480
      X2              =   3240
      Y1              =   960
      Y2              =   960
   End
   Begin VB.Line Line42 
      X1              =   3240
      X2              =   3240
      Y1              =   960
      Y2              =   1200
   End
   Begin VB.Line Line41 
      X1              =   3120
      X2              =   3240
      Y1              =   1200
      Y2              =   1200
   End
   Begin VB.Line Line40 
      X1              =   4920
      X2              =   5160
      Y1              =   840
      Y2              =   840
   End
   Begin VB.Line Line39 
      X1              =   2880
      X2              =   3120
      Y1              =   840
      Y2              =   840
   End
   Begin VB.Line Line38 
      X1              =   5160
      X2              =   5040
      Y1              =   840
      Y2              =   600
   End
   Begin VB.Line Line37 
      X1              =   4920
      X2              =   5040
      Y1              =   840
      Y2              =   600
   End
   Begin VB.Line Line36 
      X1              =   5160
      X2              =   5160
      Y1              =   2280
      Y2              =   840
   End
   Begin VB.Line Line35 
      X1              =   4920
      X2              =   4920
      Y1              =   1920
      Y2              =   840
   End
   Begin VB.Line Line34 
      X1              =   5640
      X2              =   6120
      Y1              =   600
      Y2              =   600
   End
   Begin VB.Line Line33 
      X1              =   1920
      X2              =   2400
      Y1              =   600
      Y2              =   600
   End
   Begin VB.Line Line32 
      X1              =   3120
      X2              =   3000
      Y1              =   840
      Y2              =   600
   End
   Begin VB.Line Line31 
      X1              =   2880
      X2              =   3000
      Y1              =   840
      Y2              =   600
   End
   Begin VB.Line Line30 
      X1              =   3120
      X2              =   3120
      Y1              =   2280
      Y2              =   840
   End
   Begin VB.Line Line29 
      X1              =   2880
      X2              =   2880
      Y1              =   2280
      Y2              =   840
   End
   Begin VB.Line Line28 
      X1              =   5640
      X2              =   5280
      Y1              =   1920
      Y2              =   1920
   End
   Begin VB.Line Line12 
      X1              =   5280
      X2              =   5280
      Y1              =   2280
      Y2              =   1920
   End
   Begin VB.Line Line27 
      X1              =   4920
      X2              =   4920
      Y1              =   1920
      Y2              =   2280
   End
   Begin VB.Line Line26 
      X1              =   4560
      X2              =   4560
      Y1              =   1920
      Y2              =   2280
   End
   Begin VB.Line Line25 
      X1              =   4200
      X2              =   4200
      Y1              =   1920
      Y2              =   2280
   End
   Begin VB.Line Line24 
      X1              =   3840
      X2              =   3840
      Y1              =   1920
      Y2              =   2280
   End
   Begin VB.Line Line23 
      X1              =   3480
      X2              =   3480
      Y1              =   1920
      Y2              =   2280
   End
   Begin VB.Line Line22 
      X1              =   3120
      X2              =   3120
      Y1              =   1920
      Y2              =   2280
   End
   Begin VB.Line Line21 
      X1              =   4920
      X2              =   5280
      Y1              =   2280
      Y2              =   2280
   End
   Begin VB.Line Line20 
      X1              =   4200
      X2              =   4560
      Y1              =   2280
      Y2              =   2280
   End
   Begin VB.Line Line19 
      X1              =   3480
      X2              =   3840
      Y1              =   2280
      Y2              =   2280
   End
   Begin VB.Line Line18 
      X1              =   3120
      X2              =   2760
      Y1              =   2280
      Y2              =   2280
   End
   Begin VB.Line Line17 
      X1              =   2760
      X2              =   2760
      Y1              =   1920
      Y2              =   2280
   End
   Begin VB.Line Line16 
      X1              =   4560
      X2              =   4920
      Y1              =   1920
      Y2              =   1920
   End
   Begin VB.Line Line15 
      X1              =   3840
      X2              =   4200
      Y1              =   1920
      Y2              =   1920
   End
   Begin VB.Line Line14 
      X1              =   3120
      X2              =   3480
      Y1              =   1920
      Y2              =   1920
   End
   Begin VB.Line Line13 
      X1              =   2400
      X2              =   2760
      Y1              =   1920
      Y2              =   1920
   End
   Begin VB.Line Line11 
      X1              =   5640
      X2              =   5880
      Y1              =   600
      Y2              =   120
   End
   Begin VB.Line Line10 
      X1              =   6120
      X2              =   5880
      Y1              =   600
      Y2              =   120
   End
   Begin VB.Line Line9 
      X1              =   5640
      X2              =   5640
      Y1              =   3960
      Y2              =   600
   End
   Begin VB.Line Line8 
      X1              =   1920
      X2              =   2160
      Y1              =   600
      Y2              =   120
   End
   Begin VB.Line Line7 
      X1              =   2400
      X2              =   2160
      Y1              =   600
      Y2              =   120
   End
   Begin VB.Line Line6 
      X1              =   1920
      X2              =   1920
      Y1              =   3960
      Y2              =   600
   End
   Begin VB.Line Line5 
      X1              =   6120
      X2              =   6120
      Y1              =   3960
      Y2              =   600
   End
   Begin VB.Line Line4 
      X1              =   2400
      X2              =   2400
      Y1              =   3960
      Y2              =   600
   End
   Begin VB.Label Label2 
      Caption         =   "You have to get up to the top of this 5 story house and get the ruby. Then you have to get back out safely"
      Height          =   1575
      Left            =   120
      TabIndex        =   2
      Top             =   1080
      Width           =   1095
   End
   Begin VB.Label Label1 
      Alignment       =   2  'Center
      Caption         =   "Press ""Enter"" to enter the Haunted House"
      BeginProperty Font 
         Name            =   "Comic Sans MS"
         Size            =   9.75
         Charset         =   0
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   855
      Left            =   3240
      TabIndex        =   1
      Top             =   0
      Width           =   1695
   End
   Begin VB.Shape Shape1 
      Height          =   135
      Left            =   3120
      Shape           =   3  'Circle
      Top             =   3480
      Width           =   255
   End
   Begin VB.Line Line3 
      X1              =   3360
      X2              =   3360
      Y1              =   3120
      Y2              =   3840
   End
   Begin VB.Line Line2 
      X1              =   3360
      X2              =   2880
      Y1              =   3120
      Y2              =   3120
   End
   Begin VB.Line Line1 
      X1              =   2880
      X2              =   2880
      Y1              =   3840
      Y2              =   3120
   End
End
Attribute VB_Name = "Form1"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False

Private Sub Command1_Click()
    Form1.Visible = False
    Form2.Visible = True
End Sub

Private Sub Form_Load()
    Form1.Visible = False
    Form24.Visible = True
End Sub
